...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            ...
            # Validation errors so update the form defaults with what the user has entered
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any], is_creation: bool = False) -> EnvisalinkAlarmPanel:
    ...
    if CONF_PANEL_TYPE not in data:
        if is_creation:
            # Only try this during initial setup of the device as it will fail if this is
            # called from the options flow because the EVL only allows a single concurrent
            # connection.
...
def get_user_data_schema(defaults: dict[str, Any], is_creation: bool = False):
    ...
    if is_creation:
        schema = {
            vol.Required(
                CONF_ALARM_NAME,
                default=defaults[CONF_ALARM_NAME]
            ): cv.string,
        }
    ...
    schema = schema | {
        ...
        vol.Required(
            CONF_PASS,
            default=defaults[CONF_PASS]
        ): cv.string,
        vol.Required(
            CONF_PARTITION_SET,
            default=defaults[CONF_PARTITION_SET]
        ): cv.string,
        vol.Required(
            CONF_ZONE_SET,
            default=defaults[CONF_ZONE_SET]
        ): cv.string,
        vol.Optional(
            CONF_CODE,
            description={"suggested_value": defaults[CONF_CODE]}
        ): cv.string,
        vol.Required(
            CONF_EVL_PORT,
            default=defaults[CONF_EVL_PORT]
        ): cv.port,
        vol.Required(
            CONF_EVL_DISCOVERY_PORT,
            default=defaults[CONF_EVL_DISCOVERY_PORT]
        ): cv.port,
    }
